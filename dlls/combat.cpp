...
void CGib :: SpawnRandomGibs( entvars_t *pevVictim, int cGibs, int human )
{
	...
	for ( cSplat = 0 ; cSplat < cGibs ; cSplat++ )
	{
		...
		else
		{
			...
			{
				// aliens
				pGib->Spawn("models/hgibs.mdl"); //magic nipples - real beta //pGib->Spawn( "models/agibs.mdl" );
				pGib->pev->body = RANDOM_LONG(1,ALIEN_GIB_COUNT-1);
			}
		}
...
void CBaseMonster :: GibMonster( void )
{
	...
	if ( HasHumanGibs() )
	{
		if ( CVAR_GET_FLOAT("violence_hgibs") != 0 )	// Only the player will ever get here
		{
			...
			CGib::SpawnStickyGibs(pev, pev->origin + pev-> view_ofs, 5);
...
void CBaseMonster :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	m_IdealMonsterState = MONSTERSTATE_DEAD;
	pev->solid = SOLID_NOT; //magic nipples - real beta
...
void CGib :: BounceGibTouch ( CBaseEntity *pOther )
{
	...
	else
	{
		...
		if ( m_material != matNone && RANDOM_LONG(0,2) == 0 )
		{
			...
			CBreakable::MaterialSoundRandom( edict(), (Materials)m_material, volume / 2 );
		}
	}
}
...
void CBaseEntity::TraceBleed(float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
	...
	*/
	...
	//if (flDamage < 10) //magic nipples - real beta
	{
		flNoise = 0.1;
		cCount = 1;
	}
...
void CBaseEntity::SpawnBlood(Vector vecSpot, int bloodColor, float flDamage)
{
	...
	if (pev->deadflag == DEAD_NO)
	{
		...
		{
			// normal
			UTIL_BloodGunshot(vecSpot, UTIL_RandomBloodVector(), bloodColor, 4, 100.0f);
		}
	}
